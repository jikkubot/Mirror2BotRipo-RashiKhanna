...
def leechSet(update, context):
    ...
    msg = f"<b>⚙️ Leech Type For {user_id} User is</b> "
    if (
    ...
    msg += "<b>Exists ✅</b>" if os.path.exists(path) else "<b>Not exists ❌</b>"
    buttons = button_build.ButtonMaker()
    buttons.sbutton("📁 As Document", f"doc {user_id}")
    buttons.sbutton("As Media 🖥️", f"med {user_id}")
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        os.remove(photo_dir)
        sendMessage(f"<b>✅ Custom Thumbnail Saved For <code>{user_id}</code> User 🤓</b>", context.bot, update)
    else:
        sendMessage("<b>🖼️ Reply to a Photo to Save Custom Thumbnail 🤒</b>", context.bot, update)
